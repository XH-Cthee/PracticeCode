...
bool selectSort(int array[], size_t arrLen) {
    ...
    for (int orderedNum = 0; orderedNum < arrLen; ++orderedNum) {
        int minIndex = orderedNum;//定义第一个数是最小的
        for (int i = orderedNum + 1; i < arrLen; ++i) {
            if (array[i] < array[minIndex]) {//遍历数组找打最小的然后交换下标
                minIndex = i;
            }
        }

        if (minIndex != orderedNum){//如果不相等就交换值
...
int main(){
    ...
    selectSort(array0, arrayLen);//选择排序
    printArray(array0, arrayLen);//选择排序后的数组
